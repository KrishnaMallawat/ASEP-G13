...
// Store shuffled questions and a random shape for each question
let shuffledQuestions = [];
let questionShapes = [];
let currentTargetShapeKey = null;
...
function initGame() {
    ...
    // Assign a random shape to each question for this session
    const shapeKeys = Object.keys(shapeImages).map(Number);
    questionShapes = [];
    for (let i = 0; i < shuffledQuestions.length; i++) {
        questionShapes.push(shapeKeys[Math.floor(Math.random() * shapeKeys.length)]);
    }
...
function loadRandomQuestion() {
    ...
    const levelQuestions = shuffledQuestions.slice(levelStart, levelEnd);
    ...
    if (questionsAnswered >= 4) {
        ...
        questionsAnswered = 0;
        if (currentLevel > 4) {
            // If all levels completed, reshuffle for a new round
            shuffledQuestions = questions.slice();
            shuffleArray(shuffledQuestions);
            // Assign new random shapes for each question
            const shapeKeys = Object.keys(shapeImages).map(Number);
            questionShapes = [];
            for (let i = 0; i < shuffledQuestions.length; i++) {
                questionShapes.push(shapeKeys[Math.floor(Math.random() * shapeKeys.length)]);
    ...
    // Pick a random question from current level
    const randomIndex = Math.floor(Math.random() * levelQuestions.length);
    currentQuestion = levelQuestions[randomIndex];
    correctAnswer = currentQuestion.correct;

    // Find the index of this question in the shuffledQuestions array
    const globalIndex = shuffledQuestions.indexOf(currentQuestion);
    // Assign the shape for this question
    currentTargetShapeKey = questionShapes[globalIndex];

    // Shuffle answer options for this question
    let shuffledAnswers = currentQuestion.answers.slice();
    shuffleArray(shuffledAnswers);

    questionText.textContent = currentQuestion.question;

    // Set answer buttons with shuffled answers
...
function setupEventListeners() {
    ...
    // Add quit button event listeners for all pages
    document.querySelectorAll('.quit-btn').forEach(btn => {
        btn.addEventListener('click', quitGame);
    });

    if (instructionBtn && instructionPage && backHomeBtn) {
...
function showMenuPage() {
    ...
    // Show quit button
    document.querySelectorAll('.quit-btn').forEach(btn => btn.style.display = 'block');
}
...
function startGame() {
    ...
    timeLeft = 30;
    gameActive = true;
    shapeDropRate = 600 - (currentLevel * 100);
    shapeSpeed = 4 + currentLevel;
    ...
    // Use the random target shape for this question
    targetShapeDisplay.textContent = shapeImages[currentTargetShapeKey];
...
function createFallingShape() {
    ...
    if (isCorrect) {
        shapeKey = currentTargetShapeKey;
    } else {
        // Pick a random shape that is NOT the current target
        let possibleShapes = Object.keys(shapeImages).map(Number).filter(num => num !== currentTargetShapeKey);
        shapeKey = possibleShapes[Math.floor(Math.random() * possibleShapes.length)];
    }
...
function endGame() {
    ...
    // Show the target shape for this question
    targetShapeResult.textContent = shapeImages[currentTargetShapeKey];
    targetShapeResult.style.fontSize = '70px';
    shapeNameDisplay.textContent = shapeNames[currentTargetShapeKey];
