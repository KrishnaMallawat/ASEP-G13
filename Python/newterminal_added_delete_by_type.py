...
def to_do_commands(intent, name):
    try:
        if intent == "create_folder":
            ...
            return delete_by_type(file_type)
        else:
...
def delete_by_type(file_type):
    ...
    if file_type.lower() not in type_map:
        return f"[yellow]Unsupported file type. Supported types: {', '.join(type_map.keys())}[/yellow]"
    
    extensions = type_map[file_type.lower()]
    deleted_files = []
    
    try:
        for file in os.listdir('.'):
            if any(file.lower().endswith(ext) for ext in extensions):
                if file != "command_log.txt":  # Skip command log file
                    try:
                        os.remove(file)
        ...
        return f"[red]Error deleting {file_type} files: {str(e)}[/red]"
